...
class show_prediction():
    ...
    def load_video_organize_dir(self):
        ...
        try:
            ...
            pass
        try:
            os.mkdir(self.path_to_pickle + '/pickle_files')
        except:
            pass
        root = tk.Tk()
        root.withdraw()
        ...
        self.pickle_path = askdirectory(title="Select pickle path")
        if self.csv_file:
    ...
    def loop_video(self, start_frame=80, interval=100):
        ...
        while (self.cap.isOpened()):
            if int(self.cap.get(cv2.CAP_PROP_POS_FRAMES)) - self.start_frame < interval:
                ...
                self.det_pred = int(self.determine_prediction(self.start_frame, self.end_frame - 1))
    ...
    def update_annotations(self):
        ...
        for frame in np.arange(self.start_frame,self.end_frame):
            self.annot_data = self.annot_data.append(
                {'frame': frame, 'pred': self.det_pred}, ignore_index=True)
    ...
    def save_annotations_as_pickle(self):
        ...
        self.annot_pickle_final.to_pickle(self.pickle_path + self.video_file[self.video_file.rfind('/'):-4] + '_boot{}.p'.format(self.boot_round))
        unpickled_df = pd.read_pickle(self.pickle_path + self.video_file[self.video_file.rfind('/'):-4] + '_boot{}.p'.format(self.boot_round))
        print(unpickled_df)
        print(unpickled_df.dtypes)
    ...
    def determine_last_frame(self):
        ...
        """
        start_frames = {}
        for file in glob.glob(self.pickle_path + self.video_file[self.video_file.rfind('/'):-4] + '*'):
            try:
                self.annot_pickle = pd.read_pickle(file)
                print(self.annot_pickle['frame'].iloc[-1])
                start_frames[self.annot_pickle['frame'].iloc[-1]] = file[(int(file.find('boot')) + 4):-2]
            except:
                continue
        if not any(start_frames):
            start_frames[80] = 1
        try:
            self.annot_pickle = pd.read_pickle(
                self.pickle_path + self.video_file[self.video_file.rfind('/'):-4] + '_boot{}.p'.format(self.boot_round))
            start_frame = max(start_frames, key=int) + 1
        except:
            self.annot_pickle = pd.DataFrame(columns=None, index=None)
            start_frame = max(start_frames, key=int)
            pass
        return start_frame
    ...
    def done_with_video(self):
        ...
        analyzed_frames = len(self.annot_pickle_final.index)
...
# update confusion matrix code to computer form output files
# write input to save as test data or train data
# fix header problem in dataframe (check with csv what it looks like)
